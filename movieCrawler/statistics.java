...
public class statistics {
    ...
    public static void main(String[] args){
    	String[] aa = {"爱乐之城","生化危机：终章","乘风破浪","功夫瑜伽","极限特工：终极回归"};
    	for(int i=0;i<5;i++){
    		statistics a = new statistics(aa[i]);
        	System.out.println(aa[i]);
        	System.out.println(a.statisticsContent);
        	Save.saveMovie(aa[i]+"statistics.txt",a.statisticsContent);
...
//        }
        ...
        while(matcher3.find()){
           String aa = matcher3.group(1)+"`"+matcher3.group(2).replace("</span>"," ") + "`" + matcher3.group(3) + "`" + matcher3.group(4) + "`" + matcher3.group(5) + "`" + matcher3.group(6);
           statistic.add(query+"`"+aa);
        }
...
//        }
    ...
    public void setStatisticsContent(){
    	if(statistic.size()==0){
    		String reg = "<li><a target=\"_blank\" href=\"(.*?)\".*?2017年";
    		Pattern pattern = Pattern.compile(reg);
            Matcher matcher = pattern.matcher(call(base_url + query));
            while(matcher.find()){
            	Usefull_url = matcher.group(1);
            	break;
            }
            setStatistic();
    	}
