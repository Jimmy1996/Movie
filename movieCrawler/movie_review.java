...
public class movie_review {
    ...
    public static void main(String[] args){
    	String[] aa = {"爱乐之城","生化危机：终章","乘风破浪","功夫瑜伽","极限特工3：终极回归"};
    	for(int i=0;i<5;i++){
        	movie_review a = new movie_review(aa[i]);
        	System.out.println(aa[i]);
        	System.out.println(a.reviewUrl);
        	Save.saveMovie(aa[i]+"_views_content.txt",a.reviewsContent);
        	System.out.println(a.reviewsList.get(0));
    	}
    }
    ...
    public void setReviewUrl(){
        ...
        String reg = "<span>\\[电影\\]</span>"+
        ".*?&nbsp;<a href=\"https://www.douban.com.*?sid:(.*?)}.*?>" +query+" </a>";
        Pattern pattern = Pattern.compile(reg);
        Matcher matcher = pattern.matcher(content);
        while(matcher.find()) {
            urlId = matcher.group(1).replaceAll(" ","");
            break;
        }
